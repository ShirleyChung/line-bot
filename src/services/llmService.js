...
export async function askLlmWithTools(userText, context = {}) {
  ...
  for (let round = 0; round < 5; round++) {
    ...
    for (const call of functionCalls) {
      ...
      if (["get_watch_prices", "get_stock_price", "get_etf_constituents", "get_futures_price", "get_latest_arxiv_papers", "get_itf_tournaments", "get_cnn_top_headlines", "summarize_article_url"].includes(call.name) && result?.text) {
        return {
          type: "text",
          text: result.text,
        };
